...
def render(glctx, mtx, pos, pos_idx, vtx_col, col_idx, resolution: int):
    ...
    color       = dr.antialias(color.float(), rast_out, pos_clip, pos_idx)
    return color
...
def fit_cube(max_iter          = 5000,
    ...
    vtx_col = torch.from_numpy(vtxc.astype(np.float64)).cuda()
    ...
    for rep in range(repeats):
        ...
        vtx_col_opt  = torch.tensor(vtx_col_rand, dtype=torch.float64, device='cuda', requires_grad=True)
...
def main():
    ...
    parser.add_argument('--outdir', help='specify output directory', default='temp/out')
